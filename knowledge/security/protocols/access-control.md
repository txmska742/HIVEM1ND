...
1. Enumerate the surface before testing any of it.
   Task: list every route, API handler, server action, server function, scheduled job and subscription that accepts an identifier from the caller. Search the source for route and handler definitions rather than reading the documentation, which describes the intended surface and not the reachable one. Where a machine-readable API description exists, compare it against the search: an endpoint missing from it is one nobody tested, per [api-conventions.md](../api-conventions.md).
...
6. Read the caller's identity from the session only.
   Task: search every route, handler and server action for a user identifier, an account identifier or a role read from the body, the query, the path parameters or a header and then used to decide what the caller may do. Identity comes from the server-side session or the verified token, per [sessions-and-credentials.md](../sessions-and-credentials.md).
   Time: 30 minutes. Repository, plus running application.
   Result: zero hits for an identity or role taken from the request, recorded with the search expressions, and a request that sets another account's identifier in the body acting as the session's own account, read from the store.

7. Make the default a denial.
