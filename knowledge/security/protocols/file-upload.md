...
3. Serve what was stored as data, never as code.
   ...
   Result: `X-Content-Type-Options: nosniff` and a content type from the allowlist rather than from the file, `Content-Disposition: attachment` for anything not rendered inline, and a request for an uploaded script file returning the file as bytes rather than executing it, recorded with the status and the body. The malware scan is recorded as present, with a test file it flagged, or as not applicable with the reason.
