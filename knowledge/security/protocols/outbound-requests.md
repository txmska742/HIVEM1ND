...
2. Send the metadata addresses through every one of them.
   Task: through each site found in step 1, request the cloud metadata endpoints the published guidance names, `169.254.169.254` and `metadata.google.internal`, plus loopback, private ranges, link-local ranges and the encoded and alternative notations of each.
...
3. Close the redirect and the name resolution gaps.
   ...
   Result: redirect following disabled, or the destination re-validated after each hop, proved by the redirect test returning a refusal. The resolved address is checked rather than the hostname, proved by the resolution test returning a refusal, and no handler fetches from the host named in a modified header.
