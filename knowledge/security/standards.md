...
Two changes matter to how a pass is planned. Supply chain failures entered the list in third place, which is why [supply-chain](protocols/supply-chain.md) is a protocol of its own rather than a step inside a dependency check. Server-side request forgery no longer has its own row: A01 lists CWE-918 among its notable weaknesses, alongside CWE-352 for cross-site request forgery, which is why both live in [request-forgery](protocols/request-forgery.md) and are graded as access control failures.
...
The protocols in this module are organised by what a diff looks like, not by that taxonomy, because an agent matching work against a scope needs the shape of the change and not the name of the discipline. The identifiers are for the report, so a reader outside the session can find the underlying test.
