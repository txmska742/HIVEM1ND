...
scope: any text about to be rewritten, and first in every run of humanize
trigger: manual, before any rewrite pass
repeat: once per target
inputs: the target text, the lists in boundaries.md
stop: every section is marked untouchable, in which case the run ends after step 2 and the target is reported as out of scope
...
3. Fence what stays literal inside open sections.
   Task: inside the sections marked open, list every identifier, quotation, brand name, defined term and verbatim string supplied for the work.
   Time: 5 minutes.
