...
stop: the draft asserts nothing about quality, scale or importance, in which case the run ends after step 1
report: claims with a referent against claims without, the invented items removed, the claims that contradicted a policy page, and the positioning check
...
2. Give each claim its referent.
   Task: mark each claim as carrying a number, a name or a date, or as carrying none. Cut every claim that carries none, or replace it with the fact that would have shown it.
   Time: 15 minutes.
...
3. Check that each referent exists.
   Task: trace every figure, customer name, testimonial, logo and screenshot to a file, a record or a published source. Remove each one that traces to nothing, and leave the slot visibly empty instead of filling it.
   Time: 15 minutes.
   Result: every remaining referent has a location, the count of invented items left is zero, and each emptied slot is listed.
