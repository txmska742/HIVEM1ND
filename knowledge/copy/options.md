...
- **Warm.** More casual, still serious and respectful, a little enthusiasm allowed. Pick for onboarding and first-use states, and for a product whose audience is not at work.
...
- **Fragment.** Two to four words, no period. Buttons, labels, menu items, tabs, headings.
...
Yes or no, decided once per project. *Can't* reads warmer than *cannot*, and a mixture of both reads careless.

## Emoji

- **None.** The default everywhere, including headings and bullets, where emoji as formatting is a catalogued tell (https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing).
...
For any Spanish column. Register is decided once per product, written in its rules file, and re-read before strings are called done, because a writer slips into their own dialect when writing fast (Practice).
